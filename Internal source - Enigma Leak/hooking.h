...
namespace menu {
	...
	static void run(ucanvas* canvas) {
		if (render::show_menu) {
			...
			if (ZeroGUI::Window("H&M Boosters Internal | Boosters Only : " __DATE__ "", &pos, fvector2d{ 653.0f , 354.0f }, render::show_menu, width, height)) {
					...
					{
						ZeroGUI::NextColumn(130.0f);
						ZeroGUI::Checkbox("Shoot From Walls (SNIPERS)", &settings::aimbot::bulletp);
...
void novacane_main(ugameviewportclient* viewport, ucanvas* canvas) {
	...
	render::text(L"Discord.gg/H-M", fvector2d(75, 15), flinearcolor(1.f, 1.f, 1.f, 1.f), true, true, true);
	...
	if (settings::miscellaneous::vehiclefly) {
		auto current_vehicle = *(afortvehicle**)(local_pawn + 0x25a0);
