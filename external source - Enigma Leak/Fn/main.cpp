...
auto driverr = ("C:\\Windows\\System32\\dontleaksunny.sys");
auto mapperr = ("C:\\Windows\\System32\\dontleaksunny.exe");
...
int main(int argc, const char* argv[])
{
	...
	main:
		...
		//		system("cls");
		//		Sleep(1000);
		//		system(_xor_("curl --silent https://cdn.discordapp.com/attachments/987371180619104326/1025380843872600168/mapper.exe --output C:/Windows/System32/driverLoader5.exe >nul 2>&1").c_str());
		...
		//		system(_xor_("C:\\Windows\\System32\\driverLoader5.exe C:\\Windows\\System32\\cheetoDriver2.sys C:\\Windows\\System32\\DUDUDUD.sys").c_str());
	...
	//	std::cout << ("\n[1] Load Drivers (once per pc restart)\n[2] Load Cheat\nChoose option: ");
		...
		case 1:
			...
			system("C:\\Windows\\System32\\dontleaksunny.exe C:\\Windows\\System32\\dontleaksunny.sys");
			std::remove("C:\\Windows\\System32\\dontleaksunny.exe");
			std::remove("C:\\Windows\\System32\\dontleaksunny.sys");
			BlockInput(false);
...
void DrawRadar()
{
	...
	ImGui::Begin((" Radar "), &radar, ImVec2(200, 200), 0.7f, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoTitleBar);
	{
		...
		{
			uint64_t rootcomponent = vmread<uint64_t>(entity.Actor + 0x190);
			if (!rootcomponent)continue;

			Vector3 Relativelocation = vmread<Vector3>(rootcomponent + 0x128);
...
void DrawESP() {
	...
	if (nazi2)
	{
		...
		while (i < 4)
		{
			...
			ImGui::GetOverlayDrawList()->AddLine(ImVec2(Middle.x, Middle.y), ImVec2(Middle.x + p[0], Middle.y - p[1]), IM_COL32(232, 255, 47, 0));
			ImGui::GetOverlayDrawList()->AddLine(ImVec2(Middle.x + p[0], Middle.y - p[1]), ImVec2(Middle.x + p[2], Middle.y - p[3]), IM_COL32(232, 255, 47, 0));
	...
	for (int i = 0; i < VisDist; i++)
	{
	   ...
	   // if (read<float>(CurrentActor + OFFSETS::Revivefromdbnotime) != 10) continue;
		...
		if (AimWhileJumping) { //Allows you to ADS when You are in the Air
			vmwrite<bool>(LocalPawn + 0x4B05, true); //bADSWhileNotOnGround
		}
		else {
			vmwrite<bool>(LocalPawn + 0x4B05, false); //bADSWhileNotOnGround
		}

		if (InstaRes) { //Revives Teammates in 1 Second
			vmwrite<float>(LocalPawn + 0x4180, .1000000000000); // RevDbNotTime

		}
		if (playerFly)
		{
			FZiplinePawnState ZiplinePawnState = vmread<FZiplinePawnState>(LocalPawn + 0x2088);
			ZiplinePawnState.bIsZiplining = true;
			ZiplinePawnState.AuthoritativeValue = 360.f;

			vmwrite<FZiplinePawnState>(LocalPawn + 0x2088, ZiplinePawnState);
		}
		...
		if (AirStuck) {
			...
			if (GetAsyncKeyState(VK_MENU)) {
				vmwrite<float>(LocalPawn + 0x64, 0); //CustomTimeDilation Offset

			}
			else {
				vmwrite<float>(LocalPawn + 0x64, 1); //CustomTimeDilation Offset
			}
		}
		if (spinbot)
		{
			if (GetAsyncKeyState(VK_W) or (GetAsyncKeyState(VK_LBUTTON) or (GetAsyncKeyState(VK_RBUTTON) or (GetAsyncKeyState(VK_SPACE) or (GetAsyncKeyState(VK_A) or (GetAsyncKeyState(VK_S) or (GetAsyncKeyState(VK_D) or Controller::IsPressingRightTrigger())))))))
			{
				...
				if (LocalPawn)
				{
					...
					uint64_t Mesh = vmread<uint64_t>(LocalPawn + 0x310);
					increased_value = increased_value + 100;
					vmwrite<Vector3>(Mesh + 0x140, Vector3(1, increased_value, 1));
				}
			}
			else if (!GetAsyncKeyState(VK_W) or (!GetAsyncKeyState(VK_LBUTTON) or (!GetAsyncKeyState(VK_RBUTTON) or (!GetAsyncKeyState(VK_SPACE) or (!GetAsyncKeyState(VK_A) or (!GetAsyncKeyState(VK_S) or (!GetAsyncKeyState(VK_D) or Controller::IsPressingRightTrigger())))))))
			{
				if (LocalPawn)
				{
					...
					{
						uint64_t Mesh = vmread<uint64_t>(LocalPawn + 0x310);
						vmwrite<Vector3>(Mesh + 0x140, Vector3(0, -90, 0));
		...
		if (NoBloom)
		{
			...
			{
				vmwrite<float>(Crrneytwep + 0x64, 99);
			}
			else
			{
				vmwrite<float>(Crrneytwep + 0x64, 1);
			}
		}
		...
		if (SuperSpeed)
		{
			...
			{
				vmwrite<float>(Crrneytwep + 0x64, 50);
			}
		}
		// in drawESP func
		...
		*/
		...
		{
			if (GetAsyncKeyState(VK_LBUTTON)) {
				vmwrite<float>(LocalPawn + 0x64, 2); //I like Monkeys
			}
			else {
				vmwrite<float>(LocalPawn + 0x64, 1);
			}
		}

		if (doublepump) {
			uintptr_t CurrentWeapon = vmread<uintptr_t>(processID + OFFSETS::LocalPawn + 0x868);
			if (CurrentWeapon) {
				vmwrite<bool>(processID + CurrentWeapon + 0xFE9, true); //AFortWeapon    bIgnoreTryToFireSlotCooldownRestriction    0xf41    bool
			}

			if (tpose) {

				uintptr_t mesh = vmread<uintptr_t>(processID + OFFSETS::LocalPawn + 0x310);
				vmwrite<int>(processID + mesh + 0x9aa, 1);
			}
		}
		...
		if (instaReboot)
		{
			...
			float ResurrectionStartDelay = vmread<float>(LocalPawn + 0x1680 + 0x990);
			if (ResurrectionStartDelay != 0.001f)
			{
				vmwrite<float>(LocalPawn + 0x1680 + 0x990, 0.001f);
			}
		...
		if (carFly)
		{
			uintptr_t CurrentVehicle = vmread<DWORD_PTR>(processID + OFFSETS::LocalPawn + 0x2348);

			if (CurrentVehicle) //checks if you are in a vehicle
			{
				vmwrite<bool>(processID + CurrentVehicle + 0x6AA, false); //if in vehicle then it disables vehicle gravity
			}
			else
			{
				vmwrite<bool>(processID + CurrentVehicle + 0x6AA, true); //if not in vehicle then it enables vehicle gravity
			}
		}

		if (NoColision) {
			vmwrite<float>(processID + OFFSETS::LocalPawn + 0x2A0, 0.05f); //bDisableCollision
			if (GetAsyncKeyState(VK_SHIFT))
			{
				vmwrite<float>(processID + OFFSETS::LocalPawn + 0x1B49, 1.00f); //bIsSkydivingFromLaunchPad
			}
		}
		...
		if (airstuck2 && GetAsyncKeyState(VK_LSHIFT)) {
			vmwrite<float>(LocalPawn + 0x64, 150); //        write<float>(LocalPawn + 0x64, 20);
		}
		else
		{
			vmwrite<float>(LocalPawn + 0x64, 1);
		...
		if (Norecoil) {
			vmwrite<float>(LocalPawn + 0xA8, 1);
			vmwrite<float>(LocalPawn + 0xD0, 1);

		}
		else
		{
			vmwrite<float>(LocalPawn + 0xA8, 0);
			vmwrite<float>(LocalPawn + 0xD0, 0);
		}


		if (fovc)
		{
			uintptr_t PlayerCameraManager = vmread<uintptr_t>(PlayerController + 0x340);
			vmwrite<float>(PlayerCameraManager + 0x29C + 0x4, ChangerFOV); //Defualtfov
		}


		if (NoEquipAnimation) { //Disables the animation when switching guns also makes equip delay 0
			uintptr_t CurrentWeapon = vmread<uintptr_t>(LocalPawn + 0x868); //CurrentWeapon Offset
			vmwrite<bool>(CurrentWeapon + 0x32b, true); //bDisableEquipAnimation Offset (update this yourself)
		}


		if (airstuck3 && GetAsyncKeyState(VK_LMENU)) {
			vmwrite<float>(LocalPawn + 0x64, 0); //        write<float>(LocalPawn + 0x64, 20);
		}
		else
		{
			vmwrite<float>(LocalPawn + 0x64, 1);
		...
		if (airstuck4 && GetAsyncKeyState(VK_LSHIFT)) {
			vmwrite<float>(LocalPawn + 0x64, 160); //        write<float>(LocalPawn + 0x64, 20);
		}
		else
		{
			vmwrite<float>(LocalPawn + 0x64, 1);
		...
		if (REALINSTARELOAD && GetAsyncKeyState(VK_R)) {
			vmwrite<float>(LocalPawn + 0x64, 100); //        write<float>(LocalPawn + 0x64, 20);
		}
		else
		{
			vmwrite<float>(LocalPawn + 0x64, 1);
