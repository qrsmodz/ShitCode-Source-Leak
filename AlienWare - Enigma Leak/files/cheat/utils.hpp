...
public:
	...
	Vector2 operator*(float number) const {
		return Vector2(x * number, y * number, z * number);
	}
};
...
D3DXMATRIX Matrix(Vector2 rot, Vector2 origin = Vector2(0, 0, 0))
{
...
D3DXMATRIX ToMatrix(Vector2 Rotation, Vector2 origin = Vector2(0, 0, 0));
D3DXMATRIX ToMatrix(Vector2 Rotation, Vector2 origin)
{
